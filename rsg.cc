...
Production expand(map<string, Definition>& grammar, string non_terminal = "<start>")
{
  const Production& expansion = grammar[non_terminal].getRandomProduction();
  vector<string> expanded_phrases;
  // iterate throught the production
  for(const string &word : expansion)
    {
      if( word[0] == '<' && word[word.length()-1] == '>' )
	{
	  Production sub_prod = expand(grammar, word);
	  expanded_phrases.insert(expanded_phrases.end(), sub_prod.begin(), sub_prod.end() );
	}
      else
	{
	  expanded_phrases.push_back(word);
	}
    }
  return Production(expanded_phrases);
}
...
int main(int argc, char *argv[])
{
  ...
  Production prod = expand(grammar);
  cout << "the text generated is: " << prod << endl;
